...
async function getProjectsByGene(genes){
    ...
    while (i < ide.length) {
        ...
        info = info.concat(await Promise.all( temp.map(async g => {
...
async function getTpmCountsByGenesOnProjects(genes, projects){
    ...
    while (i < ide.length) {
        var end = ((i + 15) <= ide.length) ? i + 15 : ide.length
        var temp = ide.slice(i, end)
        info = info.concat(await Promise.all( temp.map(async p => {
            result[p] = {}
            ...
            var table = data.replaceAll('\r','').split('\n').slice(1).map( e => { return e.split('\t') } )
            var files_ = []
            var count_files = table.map( e => { console.log(e); var files = {}; files['workflow_type'] = e[0]; files['case_id'] = e[1]; files['sample_id'] = e[4]; files['sample_type'] = e[5]; files['cases_sample_submitter_id'] = e[6]; files['tissue_type'] = e[7]; files['tumor_descriptor'] = e[8]; files['cases_submitter_id'] = e[9]; files['data_category'] = e[10]; files['data_type'] = e[11]; files['file_name'] = e[13]; files['file_id'] = e[14]; files_.push(files); return e[e.length-1]; } )
...
async function getTpmCountsByGenesFromFiles(genes, files){
    ...
    while (i < files.length) {
        ...
        info = info.concat(await Promise.all( temp.map(async f => {
...
async function getMafInformationFromProjects( projects){
    ...
    while (i < ide.length) {
        ...
        info = info.concat(await Promise.all( temp.map(async p => {
            ...
            var table = data.replaceAll('\r','').split('\n').slice(1).map( e => { return e.split('\t') } )
            var files_ = []
            var count_files = table.map( e => { console.log(e); var files = {}; files['case_id'] = e[0]; files['ethnicity'] = e[1]; files['gender'] = e[2]; files['race'] = e[3]; files['year_of_birth'] = Number(e[4]); files['age_at_diagnosis'] = Number((Number(e[5])/365).toPrecision(2)); files['classification_of_tumor'] = e[6]; files['case_submitter_id'] = e[13]; files['file_id'] = e[14]; files_.push(files); return e[e.length-1]; } )
...
async function getVariantInformationFromMafFiles(res){
    ...
    for (var p of projects){
        ...
        while (i < files.length) {
            ...
            info = info.concat(await Promise.all( temp.map(async f => {
                ...
                try{
                    ...
                    filter.forEach( e => {
                        var build = e[3].indexOf('37')!=-1 ? 'hg19' : e[3].indexOf('38')!=-1 ? 'hg38' : ''
                        if( build!=''){
                    ...
                    if ( files.length == gr.length) {
                        result[p]['mutational_spectra'] = await tcga.mutspec.convertMatrix( info, 100 )
                    } 
                }
                catch(e){
        ...
        //result[p]['variant_information'] = info
        //result[p]['mutational_spectra'] = tcga.mutspec.convertMatrix(info)
...
if( typeof(mutspec)=="undefined" ){
    var server = (location.host=='127.0.0.1') ? `http://${location.host}/nih/msig_yas/mutationalSpectrum.js` : `https://${location.host}/export.js`
