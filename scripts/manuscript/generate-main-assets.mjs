...
function figure1Architecture() {
  return customFigurePage({
    ...
    subtitle:
      "mSigSDK fetches public spectra, catalogs, optional public-data identifiers, and runtime assets when requested while user spectra, exposures, QC outputs, plots, and reports remain in the browser unless exported.",
...
function figure1ArchitectureSvg() {
  ...
  <g transform="translate(82 222)">
    ...
    <text x="108" y="102" text-anchor="middle" class="fig1-muted fig1-mini">public IDs only</text>
  </g>
  ...
  ${fig1ResourceItem(90, 552, DESIGN.vermillion, "UCSC context lookup", "MAF coordinates; strictLocal off")}
  <text x="198" y="612" text-anchor="middle" class="fig1-muted fig1-mini">strictLocal disables public-ID and coordinate fetches.</text>
...
function figure2ZeroInstallRedesigned(e1) {
  ...
  return customFigurePage({
    ...
    script: `
      ...
      timeline.append("text").attr("x", 0).attr("y", -20).attr("font-size", 13).attr("font-weight", 700).attr("fill", p.muted)
        .text("Cumulative time from page-load start");
      ...
      svg.append("text").attr("x", 64).attr("y", 652).attr("font-size", 13).attr("font-weight", 700).attr("fill", p.muted)
        .text("All displayed step times are cumulative from page-load start; browser launch and URL typing are excluded.");
  ...
  return customFigurePage({
    ...
    script: `
      ...
      whiskers.append("line")
        ...
        .attr("y2", (row) => y(row.q3Ms || row.medianMs))
        .attr("stroke", p.ink).attr("stroke-width", 1.7).attr("stroke-linecap", "round");
      whiskers.append("line")
        .attr("x1", -8).attr("x2", 8)
        .attr("y1", (row) => y(row.q1Ms || row.medianMs))
        .attr("y2", (row) => y(row.q1Ms || row.medianMs))
        .attr("stroke", p.ink).attr("stroke-width", 1.7).attr("stroke-linecap", "round");
      whiskers.append("line")
        .attr("x1", -8).attr("x2", 8)
        .attr("y1", (row) => y(row.q3Ms || row.medianMs))
        .attr("y2", (row) => y(row.q3Ms || row.medianMs))
        .attr("stroke", p.ink).attr("stroke-width", 1.7).attr("stroke-linecap", "round");
      svg.selectAll("text.value").data(completed).join("text")
        .attr("class", "value")
