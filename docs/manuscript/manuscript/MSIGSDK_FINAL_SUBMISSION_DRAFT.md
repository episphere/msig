...
MAF-like local rows are converted through a profile registry that preserves the legacy SBS96 path while adding direct MAF-derived targets for SBS1536, DBS78, and ID83. `userData.convertMatrix` remains the backward-compatible SBS96 wrapper, while `userData.convertMafToProfileSpectra` returns profile-keyed spectra, row-level traces, audit summaries, and warnings for each requested matrix. SBS profiles use trinucleotide or pentanucleotide sequence context from row fields, lookup tables, or the UCSC Genome Browser sequence API (`https://api.genome.ucsc.edu/getData/sequence`) for the selected genome build. DBS78 is counted from explicit DNP rows or adjacent SNV pairs, and ID83 is counted from insertion/deletion alleles with repeat or microhomology annotations when present. The `analyzeMafFiles` workflow records genome build, context source, lookup mode, API endpoint when used, fetch timestamp, cache status, profile registry metadata, and count-reconciliation checks in provenance metadata. Signature fitting runs only when the supplied catalog matches the selected profile matrix. For strict offline or high-throughput use, analysts can pre-compute profile matrices externally, supply a context lookup table, or import spectra directly through `validation.rowsToSampleSpectra` or the TSV import helpers.
