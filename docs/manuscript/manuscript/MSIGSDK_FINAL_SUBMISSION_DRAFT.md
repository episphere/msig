...
The public-cohort capability demonstration used 38 public PCAWG Lung-AdenoCA SBS96 spectra retrieved from mSigPortal and browser-side NNLS refitting against the mSigPortal COSMIC v3 GRCh37 SBS96 catalog (67 signatures) (Figure 3). The figure summarizes the cohort as manuscript-scale SDK outputs: mean normalized COSMIC exposure across the cohort for the dominant fitted signatures, with remaining signatures grouped as Other; cohort quality-control metrics and mutation-burden distribution; exposure-threshold sensitivity comparing active-signature counts with reconstruction fit across cutoffs; and exploratory rank-6 non-negative matrix factorization, including rank diagnostics and the six extracted SBS96 component profiles.
