...
We added a more balanced description of the public-API design. Bundled runtime/package artifacts are now SHA-256 verified against recorded manifests, so local package substitution or corruption is detected before use. For resources retrieved through the SDK caching layer, including the SDK mSigPortal, TCGA/GDC, and UCSC live-context helper paths, the shared fetch wrapper now computes a SHA-256 checksum over the raw response payload bytes before parsing and exposes that checksum in the returned response provenance, parsed JSON source provenance, and optional provenance log. This supports post hoc verification against the source payload that was retrieved for a report. We do not claim this is equivalent to pre-use integrity verification against an upstream signed manifest, and we do not claim it covers fetches outside the SDK caching layer. Upstream public APIs can still change, withdraw, or temporarily fail to serve data, and we now identify this as a reproducibility and availability limitation. The mitigations are scoped payload-checksum provenance, explicit endpoint documentation, and a strict-local/offline path for workflows that can use local spectra and bundled/offline context data.

### 4. Runtime discrepancy between the demo and manuscript benchmark

We thank the reviewer for identifying this discrepancy. We found that the original benchmark mixed scopes. The expanded benchmark now labels Figure 4 as **native-JavaScript exposure-solve scenarios only** and reports individual observations, minima/maxima, IQR, p95, medians, separate cold/warm phases, component fields, and observed JavaScript heap from 20 isolated repeats per browser. These data were generated for Chrome, Edge, and Firefox on the available Windows host.
...
We also corrected the zero-install timing figure to label steps as cumulative from page-load start and to show the exact execution path. The current fresh-profile Chrome 150.0.7871.187 run is 1.815 s to rendered-report readiness; the separately timed stages are SDK/module import 1,476 ms, public-fetch critical path 300.45 ms (spectrum 224.20 ms; catalog 300.14 ms), native NNLS 12.625 ms, QC evidence 2.205 ms, report serialization 0.700 ms, DOM report rendering 13.760 ms, and observed JavaScript heap 32,424,646 bytes (about 30.9 MiB). The run did not initialize Pyodide/WebR or import a wrapped package. These are run-specific observations; the earlier 4.993 s and 6.896 s observations were superseded by this rerun, and their variation illustrates why the import/fetch stages are reported separately.
...
We thank the reviewer for emphasizing that adapter fidelity alone does not demonstrate the scientific value of a common multi-tool layer. We added a ground-truth benchmark using the peer-reviewed synthetic SBS dataset from Islam et al. (2022), distributed in the benchmark archive of Díaz-Gay et al. (2023) ([Islam et al.](https://pubmed.ncbi.nlm.nih.gov/36388765/); [Díaz-Gay et al.](https://academic.oup.com/bioinformatics/article/39/12/btad756/7473371); [Figshare archive](https://doi.org/10.6084/m9.figshare.24457114.v1)). We did not generate synthetic spectra locally.

The published archive contains 2,700 synthetic spectra, 300 for each of nine cancer types, generated from 21 COSMIC reference signatures with 0%, 5%, and 10% noise conditions. For a bounded executable comparison, we selected the first archived spectrum from each cancer-type group, giving nine spectra per noise level while preserving the source spectra, catalog, cancer-type strata, and published true activities. The archived SBS catalog contains 78 signature columns; we used that shape without adding or imputing signatures. The subset and archive checksum are recorded in `docs/manuscript/experiments/e7_four_tool_comparison/published_synthetic_benchmark/published-synthetic-benchmark-provenance.json`.

The same spectra and the same 78-column catalog were passed to all four adapters: deconstructSigs 1.8.0, sigminer 2.3.1, SigProfilerAssignment 1.1.3, and MuSiCal 1.0.0. The comparison used local pinned package environments; the browser adapters were not used for this benchmark because the WebR deconstructSigs process did not advance within the bounded runtime test. This is separate from the browser-versus-local adapter-fidelity result reported above.

Before comparison, we harmonized the outputs as follows:

- deconstructSigs, sigminer, and SigProfilerAssignment relative outputs and MuSiCal count outputs were all converted to relative fractions over the complete 78-signature catalog;
- every catalog column was normalized to unit sum before reconstruction;
- missing or inactive signatures were retained as zero;
- a common post-fit cutoff of 0.01 was applied, values below the cutoff were set to zero, and retained exposures were renormalized;
- a predicted signature was active when its harmonized fraction was at least 0.01, while a truth signature was active when its published activity was greater than zero;
- numerical metrics retained full floating-point values before reporting, and all tools used the same reconstruction cosine metric.

Against the archived true exposures, mean exposure-vector cosine similarity at 0% noise was 0.9984 for deconstructSigs, 1.0000 for sigminer, 1.0000 for SigProfilerAssignment, and 0.9987 for MuSiCal. At 5% noise, the corresponding values were 0.9882, 0.9922, 0.9996, and 0.9984; at 10% noise they were 0.9724, 0.9809, 0.9894, and 0.9926. Mean F1 for active-signature calls versus all nonzero archived activities was 0.902, 0.991, 0.991, and 0.945 at 0% noise, falling to 0.571, 0.564, 0.957, and 0.917 at 10% noise. At 10% noise, mean active-signature precision was 0.446 for deconstructSigs, 0.447 for sigminer, 1.000 for SigProfilerAssignment, and 0.929 for MuSiCal. Mean reconstruction cosine remained at least 0.996 for every tool and noise level, showing that reconstruction quality alone is less discriminating than exposure accuracy and active-signature recovery in this benchmark.

Pairwise exposure correlations ranged from 0.9982 to 1.0000 at 0% noise, 0.9921 to 0.9979 at 5% noise, and 0.9584 to 0.9895 at 10% noise. The largest cross-tool exposure ranges at 0% noise were SBS5 (mean range 0.0268), SBS40 (0.0156), SBS6 (0.0047), SBS1 (0.0046), and SBS3 (0.0029). Descriptive burden-stratified analysis used fixed tertiles of the archived noise-free mutation counts (mean burdens 3,612, 8,007, and 30,207 mutations). At 10% noise and the high-burden tertile, mean exposure cosine was 0.9405 for deconstructSigs, 0.9836 for sigminer, 0.9697 for SigProfilerAssignment, and 0.9967 for MuSiCal; the corresponding F1 values were 0.524, 0.687, 0.907, and 0.952. These burden results are descriptive because each tertile contains only three spectra.

The new figure is `docs/manuscript/experiments/e7_four_tool_comparison/figures/figure-e7-published-synthetic-benchmark.svg`. Machine-readable outputs include `published-synthetic-benchmark-summary.csv`, `published-synthetic-benchmark-by-burden.csv`, `published-pairwise-comparison.csv`, `published-synthetic-sample-metrics.csv`, and `published-signature-discrepancies.csv` under `docs/manuscript/experiments/e7_four_tool_comparison/published_synthetic_benchmark/`; the reproducible entry point is `docs/manuscript/experiments/e7_four_tool_comparison/code/analyze-published-synthetic-benchmark.mjs`.

We also revised the manuscript interpretation: matching context order and matrix shape are necessary but not sufficient for semantic comparability. The adapter layer standardizes input/output structure and evaluation, while optimization objectives, sparsity and signature-selection behavior, numerical implementations, and package defaults remain tool-specific. Because this executed benchmark uses nine spectra per noise level rather than all 2,700 archived spectra, we present it as evidence that the common layer enables ground-truth comparison and sensitivity analysis, not as a universal ranking of the underlying tools.
...
We added pinned runtime versions, package manifests, integrity verification, schema validation examples, benchmark artifacts, and a supplement bundle. Internal solver checks were rerun with 12 paired NMF restarts using shared positive W/H initialization matrices, matched Frobenius/MU settings, a 10,000-iteration cap, tolerance 1e-7, and exact one-to-one component matching: NNLS agreed with SciPy within `1.6507328837178648e-9`, NNLS agreed with R `nnls` within `1.6511876310687512e-9`, all 12 paired NMF runs converged with median SDK/reference reconstruction-error ratio `1.0000062087666617` and median matched-component cosine `0.9999998828504884`, and the QC metric delta was `7.771561172376096e-16`.
...
The expanded exposure-solve benchmark is complete for the available Windows browsers: Chrome, Edge, and Firefox; five native-JavaScript scenarios; cold and warm phases; 20 isolated repeats per browser/scenario/phase; individual observations and ranges; component fields; and observed JavaScript heap where exposed. Figure 4 now presents individual observations, minima/maxima, medians, and IQR, rather than a median alone. The E4 summary includes `max_observed_peak_js_heap_bytes` for each browser/scenario/phase group where exposed, while Firefox values are unavailable rather than zero. The component-boundary table explicitly labels Pyodide/WebR initialization, wrapped-package import, public spectrum/catalog fetch, adapter fitting, and plot rendering as not applicable or not measured for E4.

Figure 2 now labels zero-install timings as cumulative from page-load start and identifies native JavaScript NNLS, the fresh-profile/no-store cache state, D3 as visualization-only, and the absence of Pyodide/WebR/wrapped-package loading. The current generated value is 1.815 s to rendered-report readiness in the fresh-profile Chrome run, with separate import, public-fetch, native-fit, QC, report-serialization, DOM-render, and observed-heap fields.
...
- Do not claim all public API responses are hash-verified or pre-verified before use; bundled runtime/package artifacts are integrity-verified before install, while resources retrieved through the SDK caching layer get post hoc payload-checksum provenance.
