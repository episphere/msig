...
| `adapters.createInteroperabilityBundle({ spectra, signatures }, options)` / `createInteroperabilityBundle(...)` | Sample spectra, optional signature catalog, context order, and included tool names. | Matched handoff bundles for SigProfilerAssignment, deconstructSigs, sigminer, and MuSiCal when inputs are available. | Uses the same context order across supported executable package adapters so outputs can be compared after package execution. |
