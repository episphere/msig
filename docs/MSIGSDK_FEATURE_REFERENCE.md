...
| `adapters` | External-tool adapters for SigProfilerAssignment, SigProfilerExtractor, deconstructSigs, and MuSiCal-compatible refit workflows. |
...
| Feature | Inputs and parameters | Output | Method and interpretation |
|---|---|---|---|
| `adapters.createInteroperabilityBundle({ spectra, signatures }, options)` / `createInteroperabilityBundle(...)` | Sample spectra, optional signature catalog, context order, and included tool names. | Matched handoff bundles for SigProfilerAssignment, SigProfilerExtractor, deconstructSigs, and MuSiCal when inputs are available. | Uses the same context order across tools so outputs can be compared after external execution. |
