...
| `adapters` | Exact executable adapters for SigProfilerAssignment, MuSiCal, deconstructSigs, and sigminer; SigProfiler-style matrix import/export is documented under `io` as a file format. |
...
|---|---|---|---|
| `workflows.analyzeMafFiles(mafFiles, signatures = null, options = {})` | Convert MAF-like rows into COSMIC-style profile spectra with optional known-signature fitting. | MAF rows or nested row arrays, optional signature catalog, `profiles`, `groupBy`, `genome`, `offline`, `contextLookupTable`, `expectedContexts`, and `reportFormat`. Default `profiles` is `["SBS96"]` for legacy compatibility; direct MAF-derived targets include `SBS96`, `SBS1536`, `DBS78`, and `ID83`. | MAF conversion metadata, `spectraByProfile`, `traceByProfile`, selected-profile `spectra`, provenance, validation/QC, and optional signature-fit analysis. Fitting runs only when the supplied signature catalog matches the selected profile matrix. |
...
|---|---|---|---|
| `convertMafToProfileSpectra(data, options = {})` | MAF-like rows or nested row arrays, `profiles`, `groupBy`, `genome`, `tcga`, `offline`, and optional `contextLookupTable`. Native MAF-derived targets are `SBS96`, `SBS1536`, `DBS78`, and `ID83`; profile definitions also record non-MAF targets that require annotated segment, SV, rearrangement, RNA, or other native input tables. | `{ spectraByProfile, traceByProfile, audit, warnings, profileRegistry }`, keyed by canonical profile name. | SBS profiles use 3-base or 5-base sequence context; DBS78 uses explicit DNP rows or adjacent SNV pairs; ID83 uses insertion/deletion alleles plus repeat or microhomology annotations when present. Every profile returns counted/skipped rows and exact skip reasons. |
| `convertMatrix(MAFfiles, groupBy = "project_code", batch_size = 100, genome = "hg19", tcga = false, options = {})` | MAF-like rows or nested row arrays, grouping field, batch size, genome build, TCGA field mapping flag, and context options. `options.offline = true` uses row-supplied contexts, a supplied `contextLookupTable`, or bundled sparse smoke-test lookup assets for hg19, hg38, or T2T-CHM13. | Sample-by-SBS96 spectra. | Backward-compatible wrapper around `convertMafToProfileSpectra(..., { profiles: ["SBS96"] })`. Live mode retrieves sequence context through the UCSC Genome Browser sequence endpoint. Offline mode requires a matching position-indexed lookup table for production-scale conversion. |
...
| `plotCosmicProfile(divID, spectra, options = {})` | One context-keyed spectrum or sample-keyed spectra object plus `profileKey`, `profile`, `matrix`, `sample`, `contexts`, title/subtitle, and figure-context metadata. | COSMIC-style plot using the renderer matched to the active profile matrix. | Dispatches `SBS96`, `SBS1536`, `DBS78`, `ID83`, and rearrangement-style profiles to profile-specific renderers. Exposed under `qcPlots` and `userData` for notebooks and reports. |
...
| `plotPatientMutationalSpectrumuserData(mutationalSpectra, matrixSize = 96, divID = "mutationalSpectrumMatrix", options = {})` | Local mutational spectra and plot settings. `options.profileKey`, `options.profile`, or `matrixSize` selects non-SBS renderers when possible. | Mutational profile plot for local user spectra. | Dispatches non-SBS profile keys through `plotCosmicProfile`; generic grouped bars remain a fallback for unsupported profile shapes. |
...
| `getExpectedContexts({ profile = "SBS", matrix = 96 })` | Profile family and matrix size. Supported committed targets include SBS96, SBS1536, DBS78, and ID83; unsupported or annotated-input-only combinations return null when no canonical context list is committed. | Ordered context labels or null. |
| `listProfileDefinitions()` | No arguments. | Registered profile targets with canonical key, matrix size, input requirements, conversion support, context count, and renderer mapping. |
| `listMafConvertibleProfiles()` | No arguments. | Native MAF-derived targets currently exposed by the converter: SBS96, SBS1536, DBS78, and ID83. |
...
| `plotCosmicProfile(divID, spectra, options)` | One context-keyed spectrum or sample-keyed spectra object plus profile key, sample, context order, title/subtitle, and figure context. | Profile-specific COSMIC-style renderer for SBS96, SBS1536, DBS78, ID83, and rearrangement-style matrices, with Plotly download controls and standalone figure context. |
...
The executable package roster is available from `adapters.listPackageRuntimes()` and currently includes SigProfilerAssignment, MuSiCal, deconstructSigs, and sigminer with pinned versions and runtime metadata.
...
| `adapters.createInteroperabilityBundle({ spectra, signatures }, options)` / `createInteroperabilityBundle(...)` | Sample spectra, optional signature catalog, context order, and included tool names. | Matched handoff bundles for SigProfilerAssignment, deconstructSigs, sigminer, and MuSiCal when inputs are available, plus a machine-readable comparison contract. | The contract records context ordering, output-unit conversion, complete-catalog zero filling, filter/renormalization order, unassigned/inactive/omitted handling, fixed options, and package-default boundaries. A common matrix shape does not imply equivalent fitting semantics. |
...
| `adapters.musical.runRefit({ spectra, signatures }, options)` / `runMuSiCalRefit(...)` | `runtime = "pyodide"`, pinned MuSiCal wheel metadata, Pyodide package list, optional timeout, and prepared spectra/signature inputs. | Exact Pyodide package result with parsed exposure matrix, prepared input manifest, collected files, raw run metadata, and provenance. | Executes the actual Park Lab MuSiCal Python package only. If the pinned wheel or runtime is unavailable, the adapter returns an explicit availability error rather than running a JavaScript substitute. |
...
- Structured object fields: `schemaVersion = "msig.report.v0.3"`, title, summary, generated timestamp, workflow role, scope statement, method basis, primary interpretation fields, reproducibility statement, structured `reproducibility` status/evidence, parameters, validation, QC, summarized signatures, summarized exposures, extraction, provenance, citations, and notes. The default record supports auditability and portability; it does not claim computational rerun support when immutable execution evidence is missing.
